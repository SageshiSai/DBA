...
SELECT COUNT(*) AS "profesores Educación" FROM profesor p, persona pp, departamento d WHERE pp.id = p.id_profesor AND p.id_departamento=d.id AND d.nombre = "Educación";

INNER JOIN

1.Lista el nombre de los departamentos y los identificadores de los profesores que trabajan en ellos.
SELECT nombre, id_profesor FROM departamento INNER JOIN profesor ON departamento.id = profesor.id_profesor;
2. Lista el dni de las personas y las asignaturas que cursan.
SELECT nif, asignatura.nombre FROM asignatura INNER JOIN alumno_se_matricula_asignatura ON asignatura.id = alumno_se_matricula_asignatura.id_asignatura
...
SELECT d.nombre, COUNT(*) FROM profesor p, departamento d WHERE p.id_departamento = d.id GROUP BY d.nombre;
RIGHT JOIN
...
SELECT asignatura.nombre, grado.* FROM 
